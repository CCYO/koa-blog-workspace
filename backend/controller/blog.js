...
/* CONFIG     ----------------------------------------------------------------------------- */
const { CONSTANT, CACHE, ERR_RES, ENV, PAGINATION } = require("../config");
...
async function findListForPagination(opts) {
  ...
  let data;
  if (opts.show) {
    data = { [CONSTANT.BLOG.STATUS.PUBLIC]: resModel.data };
  } else {
    data = { [CONSTANT.BLOG.STATUS.PRIVATE]: resModel.data };
  }
...
async function findListAndCountOfSquare(opts) {
  ...
  let data = {
    [CONSTANT.BLOG.STATUS.PUBLIC]: { blogs, count },
  };
  return new SuccModel({ data });
}
...
async function findListAndCountOfAlbum(opts) {
  ...
  let data = {};
  if (public) {
    data[CONSTANT.BLOG.STATUS.PUBLIC] = await Blog.readListAndCountAll(
      Opts.BLOG.FIND.listAndCountForAlbum({ ...opts, show: true })
    );
  }
  if (private) {
    data[CONSTANT.BLOG.STATUS.PRIVATE] = await Blog.readListAndCountAll(
