...
/* CONFIG     ----------------------------------------------------------------------------- */
const { CONSTANT, ERR_RES, SERVER } = require("../config");
...
/* VAR        ----------------------------------------------------------------------------- */
const pattern = `^(${CONSTANT.AJV.IMG.EXT.map((ext) => `(${ext})`).join(
  "|"
)})$`;
...
async function addBlogImg(ctx) {
  ...
  if (!exist) {
    ...
    let { files } = await _parse(ctx, {
      maxFileSize: CONSTANT.AJV.BLOG.IMG.MAX_SIZE,
    });
...
async function addUserAvatar(ctx) {
  ...
  let { fields, files } = await _parse(ctx, {
    maxFileSize: CONSTANT.AJV.SETTING.AVATAR.MAX_SIZE,
  }).catch((e) => {
    throw e;
  });
  if (
