...
async function readList({ user_id, excepts }) {
  ...
  function genQuery(user_id, excepts) {
    ...
    FROM (
        SELECT ${CONSTANT.NEWS.TYPE.IDOL_FANS} as type, id , idol_id as target_id , fans_id as follow_id, confirm, createdAt, updatedAt
        FROM IdolFans
        WHERE 
            idol_id=${user_id}
        ...
        SELECT ${CONSTANT.NEWS.TYPE.ARTICLE_READER} as type, id, article_id as target_id, reader_id as follow_id, confirm, createdAt, updatedAt 
        FROM ArticleReaders
        WHERE 
            reader_id=${user_id}
            AND deletedAt IS NULL
        ...
        SELECT ${CONSTANT.NEWS.TYPE.MSG_RECEIVER} as type, id, msg_id as target_id, receiver_id as follow_id, confirm, createdAt, updatedAt 
        FROM MsgReceivers
        WHERE 
            receiver_id=${user_id}
            AND deletedAt IS NULL
  ...
  function genQuery(user_id) {
    return `
      SELECT
          ...
          COUNT(*) as total
      FROM (
          SELECT ${CONSTANT.NEWS.TYPE.IDOL_FANS} as type, id, confirm
          FROM IdolFans
          WHERE
              idol_id=${user_id} 
              AND deletedAt IS NULL 
          ...
          SELECT ${CONSTANT.NEWS.TYPE.ARTICLE_READER} as type, id, confirm
          FROM ArticleReaders
          WHERE 
              reader_id=${user_id}
              AND deletedAt IS NULL 
          ...
          SELECT ${CONSTANT.NEWS.TYPE.MSG_RECEIVER} as type, id, confirm
          FROM MsgReceivers
          WHERE
              receiver_id=${user_id}
              AND deletedAt IS NULL 
