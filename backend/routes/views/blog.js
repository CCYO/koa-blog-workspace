 ...
 */
router.get("/preview/:id", CHECK.validParam, privateCache, async (ctx) => {
  let opts = {
    //  來自 privateCache
    cache: ctx.cache,
 ...
 */
router.get("/edit/:id", CHECK.validParam, privateCache, async (ctx, next) => {
  let opts = {
    //  來自 privateCache
    cache: ctx.cache,
    author_id: ctx.session.user.id,
 ...
 * @description blog
 */
router.get("/:id", CHECK.validParam, commonCache, async (ctx) => {
  let opts = {
    //  來自 privateCache
    cache: ctx.cache,
    blog_id: ctx.params.id * 1,
