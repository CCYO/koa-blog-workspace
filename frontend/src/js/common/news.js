      ...
      update: {
        value(list) {
          for (let { type, id } of list) {
            let prop;
            switch (type) {
              case CONSTANT.NEWS.TYPE.IDOL_FANS:
                prop = "idolFans";
                break;
              case CONSTANT.NEWS.TYPE.ARTICLE_READER:
                prop = "articleReader";
                break;
              case CONSTANT.NEWS.TYPE.MSG_RECEIVER:
                prop = "msgReceiver";
                break;
            }
            this[prop].push(id);
    ...
    Object.defineProperty(this, "status", {
      get() {
        if (this.#first) {
          ...
          return {
            status: CONSTANT.NEWS.STATUS.AGAIN,
  ...
  async init({ me, news: newsData }) {
    ...
    if (WebSocket) {
      ...
      ws.onclose = async (event) => {
        ...
        if (event.code >= CONSTANT.WS.CLOSE.EMPLOYER_ID.CODE) {
          if (!document.hidden) {
            logout(event.reason);
          } else {
            ins_news.ws_cb = () => {
      ...
      ws.onmessage = async function (event) {
        switch (Number(event.data)) {
          case CONSTANT.WS.HAS_UNCONFIRM:
            await getNews();
            break;
          case CONSTANT.WS.HAS_CONFIRM:
...
class Render {
  ...
  async _countFadeIn(first) {
    ...
    if (
      newCount === 0 ||
      (first && htmlStr.unRender.total > CONSTANT.NEWS.LIMIT)
    ) {
  ...
  async _renderFadeIn() {
    ...
    let scroll =
      ...
      htmlStr.unRender.unconfirm <= CONSTANT.NEWS.LIMIT;
    htmlStr.render();
    if (scroll) {
