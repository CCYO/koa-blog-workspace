...
export default class {
  ...
  #ACTIVE_WHITE_LIST = [
    //  不允許登入權限的頁面
    CONSTANT.PAGE.REGISTER_LOGIN.ACTIVE.REGISTER,
    CONSTANT.PAGE.REGISTER_LOGIN.ACTIVE.LOGIN,
    //  不需要登入權限的頁面
    CONSTANT.PAGE.SQUARE.ACTIVE._,
    CONSTANT.PAGE.USER.ACTIVE.OTEHR,
    CONSTANT.PAGE.BLOG.ACTIVE._,
    CONSTANT.PAGE.BLOG.ACTIVE.PREVIEW,
    CONSTANT.PAGE.ERR_PAGE.ACTIVE.NODE_JS,
    CONSTANT.PAGE.ERR_PAGE.ACTIVE.NGINX,
  ];
  ...
  constructor({ backdrop = undefined, active }) {
    ...
    instance.interceptors.response.use(
      (response) => {
        ...
        if (
          resData.errno === CONSTANT.ERR_RES.AXIOS.PAGINATION_UPDATE ||
          //  後端需要登入權限的請求，響應 noLogin 提醒
          resData.errno === CONSTANT.ERR_RES.AXIOS.RESPONSE_NO_LOGIN.errno ||
          //  後端針對news請求，響應 noLogin 提醒
          (resData.errno === CONSTANT.ERR_RES.AXIOS.NEWS_NO_LOGIN.errno &&
