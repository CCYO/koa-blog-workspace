...
module.exports = {
  apply(compiler) {
    compiler.hooks.afterEmit.tap("create_error_page", async (compilation) => {
      await _create_nginx_error_pages();
    });
  },
};
//   生成NGINX響應的ERROR PAGE
async function _create_nginx_error_pages() {
  ...
  let map = Object.entries(CONSTANT.ERR_RES.VIEW);
  // map [[status, errModel], ... ]
  return Promise.all(
    map.map(([status, errModel]) => {
      ...
      fs.writeFileSync(
        ...
        ejs.render(
          template,
          {
            active: CONSTANT.PAGE.ERR_PAGE.ACTIVE.NGINX,
            page: CONSTANT.PAGE.ERR_PAGE.PAGE_NAME,
            login: false,
            errModel,
          },
          {
